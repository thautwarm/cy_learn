    ...
    def interpret(self, parsed):
        ...
        if not parsed['处理文件']:
            ...
            if len(files) is not 1:
                warn.append('输入多个文件excel文件时, 请保证各个文件的字段名一致\n')
                self.console.insert(END, "Warning:" + warn[-1])
                old = pd.concat([remove_undef_col(pd.read_excel(file[0],sheetname=0 if len(file) is 1 else file[1])) 
                            for file in map(lambda _: _.split('表名为'), files)])
            else:
                file = files[0]
                res = list(map(str.strip, file.split('表名为')))
                file = res[0]
                sheet = 0
                if len(res) > 1:
                    sheet = res[1]
                old = remove_undef_col(pd.read_excel(file, sheetname=sheet))
        ...
        for outfile, arrs in GroupBy(new.values, fn_group).items():
            ...
            pd.DataFrame(arrs, columns=total_arg_tps).to_excel(outfile, encoding='gbk')
