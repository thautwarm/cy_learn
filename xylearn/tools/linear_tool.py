...
class StatsEnv:
    """
    StatsEnv(df, target='Health (Life Expectancy)', t=0.05, digit=8)
    """
    ...
    def context_format(self):
        ...
        return dict(
            ...
            interval=format_df(pd.DataFrame([context.data.columns[1:], context.interval]).T,
                               ['coef for', '(inf, sup)']),
            coef_checked_pass=format_df(pd.DataFrame([context.data.columns[1:], context.coef_checked_pass]).T,
                                        ['coef for', 'is_passed']))
